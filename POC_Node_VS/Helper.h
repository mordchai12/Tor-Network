...
public:
	...
	static std::vector<unsigned char> getPartFromSocketVec(const SOCKET sc, const int bytesNum, const int flags);
	static uint32_t readMessageLength(SOCKET sc);
	static std::vector<unsigned char> receiveExact(SOCKET sc, size_t bytesNum);
	static void sendEncryptedData(const SOCKET sc, const std::vector<unsigned char>& encryptedData);
